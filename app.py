...
st.write("➡️ Upload an image with **less than 100 parking slots** for best results.")

# === Cache model loading to reduce boot time ===
@st.cache_resource
def load_model():
...
st.write("### 🖼 Choose a Default Image")
cols = st.columns(4)
buttons = []
    ...
    with cols[i]:
        if Path(path).exists():
            ...
            if img is not None:
                ...
                st.image(img_rgb, caption=label)
            else:
...
# === Upload Custom Image ===
uploaded_file = st.file_uploader("📤 Upload a custom parking lot image", type=["jpg", "jpeg", "png"])
...
if image_path:
    ...
    if uploaded_file and image_path and Path(image_path).exists():
        try:
            ...
            st.warning(f"⚠️ Temp file cleanup failed: {str(e)}")
